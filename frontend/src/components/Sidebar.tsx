...
export function Sidebar({ refreshKey }: { refreshKey: number }) {
  ...
  // Re-decide open/closed on every crossing of the lg breakpoint — a window
  // resize, a tablet rotation, or a browser snapped to half-screen must not
  // leave the sidebar stuck in whichever state it happened to mount in. Below
  // lg it renders as an overlay (see the fixed positioning below), so this
  // only resets it to the same default a fresh visit at that width would get.
  useEffect(() => {
  ...
  if (!open) {
    return (
      <button
        ...
        className="fixed left-3 top-3 z-20 flex h-11 w-11 items-center justify-center rounded-lg text-[var(--color-muted)] hover:bg-[var(--color-surface)] hover:text-[var(--color-text)]"
  ...
  return (
    <>
      {/* Below lg the sidebar overlays the page instead of squeezing it into a
          sliver — a static flex layout at phone widths left ~110px for the
          main content, wrapping every heading into an unreadable column. */}
      <div
        onClick={() => setOpen(false)}
        aria-hidden="true"
        className="fixed inset-0 z-10 bg-black/50 lg:hidden"
      />
      <aside className="fixed inset-y-0 left-0 z-20 flex w-[260px] shrink-0 flex-col border-r border-[var(--color-border)] bg-[var(--color-surface)] lg:static">
        <div className="flex items-center justify-between px-3 py-3">
          ...
          <button
            onClick={() => setOpen(false)}
            className="flex h-11 w-11 items-center justify-center rounded-lg text-[var(--color-faint)] hover:text-[var(--color-text)]"
